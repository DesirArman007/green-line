...
// Sub-component to handle state per route group
function RoutePricingGroup({ routeName, items }: { routeName: string, items: PricingData[] }) {
  const [selectedIdx, setSelectedIdx] = useState(0);
  // Default to first item if exists
  const currentData = items.length > 0 ? items[selectedIdx] : null;

  if (!currentData) return null;

  const handleWhatsAppRedirect = () => {
    const message = `Hello GreenLine, I would like to book a ${currentData.name} for ${routeName} route. Please send details.`;
      ...
      <div className={styles.layoutGrid}>
        ...
        <div className={styles.posterCol}>
          <AnimatePresence mode="wait">
            ...
            >
              <img
                src={currentData.posterImage || '/images/posters/tall_poster_v2.png'}
...
export default function RoutePricing() {
  const [groupedRoutes, setGroupedRoutes] = useState<Record<string, PricingData[]>>({});
  ...
  useEffect(() => {
    async function fetchData() {
      const { data, error } = await supabase.from('route_pricing').select('*').order('created_at', { ascending: true });
      if (!error && data) {
        // Map DB fields to our PricingData interface
        const mappedData: PricingData[] = data.map((row: any) => ({
          id: row.id,
          name: row.name,
          route_name: row.route_name || 'VIJAYAWADA ⇄ HYDERABAD',
          posterImage: row.poster_image,
          oneWay: {
            price: row.one_way_price,
            extraKm: row.per_extra_km,
            extraHr: row.per_extra_hour,
            limit: row.one_way_day_limit
          },
          roundTrip: {
            perKm: row.round_trip_per_km,
            batta: row.driver_batta,
            calc: row.round_trip_day_limit
          }
        }));

        // Group by route_name
        const groups: Record<string, PricingData[]> = {};
        mappedData.forEach(item => {
          if (!groups[item.route_name]) {
            groups[item.route_name] = [];
          }
          groups[item.route_name].push(item);
        });
  ...
  if (isLoading) {
    return <section className={styles.section} id="route-pricing"><div className={styles.container}>Loading routes...</div></section>;
  }

  const routeNames = Object.keys(groupedRoutes);

  return (
    <section className={styles.section} id="route-pricing">
      {routeNames.length > 0 ? (
        routeNames.map((routeName, idx) => (
          <RoutePricingGroup key={idx} routeName={routeName} items={groupedRoutes[routeName]} />
        ))
      ) : (
        <div className={styles.container}>No routes configured.</div>
      )}
    </section>
  );
}
